...
describe('XCAmpleforthController:Initialization', () => {
  ...
  it('should initialize the token reference', async function () {
    expect(await controller.xcAmpl()).to.eq(mockToken.address);
  });
...
describe('XCAmpleforthController:setOrchestrator', async () => {
  ...
  it('should NOT be callable by non-owner', async function () {
    expect(controller.connect(accounts[5]).setOrchestrator(orchestrator)).to.be
      .reverted;
  });

  it('should be callable by owner', async function () {
    expect(controller.connect(deployer).setOrchestrator(orchestrator)).to.not.be
      .reverted;
  });

  it('should update the Orchestrator reference', async function () {
    expect(await controller.orchestrator()).to.be.eq(
      ethers.constants.AddressZero,
    );
    await controller.connect(deployer).setOrchestrator(orchestrator);
    expect(await controller.orchestrator()).to.be.eq(orchestrator);
  });
});
