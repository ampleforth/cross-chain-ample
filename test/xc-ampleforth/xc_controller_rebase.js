...
let accounts, deployer, bridge, policy, mockToken, mockOrchestrator;
async function setupContracts() {
  ...
  await policy.connect(deployer).setOrchestrator(ethers.constants.AddressZero);
}
...
describe('XCAmpleforthController:rebase:accessControl', async () => {
  ...
  it('should NOT be callable by non-bridge', async function () {
    await expect(policy.connect(deployer).rebase(2, 1000)).to.be.revertedWith(
      'XCAmpleforthController: Bridge gateway not whitelisted',
    );
  });

  it('should be callable by bridge', async function () {
    await expect(policy.connect(bridge).rebase(2, 1000)).to.not.be.reverted;
  });
});
...
describe('XCAmpleforthController:rebase:epoch', async () => {
  ...
  describe('when epoch is not new', function () {
    it('should revert', async function () {
      await expect(policy.connect(bridge).rebase(1, 1000)).to.be.revertedWith(
        'XCAmpleforthController: Epoch not new',
      );
    });
  });
...
describe('XCAmpleforthController:rebase', async () => {
  ...
  it('should NOT allow successive invocation', async function () {
    await policy.connect(bridge).rebase(2, 50626634);
    await expect(policy.connect(bridge).rebase(2, 50626634)).to.be.revertedWith(
      'XCAmpleforthController: Epoch not new',
    );
    await increaseTime(3600);
    await expect(policy.connect(bridge).rebase(2, 50626634)).to.be.revertedWith(
...
describe('XCAmpleforthController:rebase:contraction', async () => {
  ...
  it('should log Rebase with supply delta', async function () {
    const r = await policy.connect(bridge).rebase(2, 22692382);
    const t = await policy.rebaseTimestampSec();
    expect((async () => r)())
      .to.emit(policy, 'LogRebase')
...
describe('XCAmpleforthController:rebase:noChange', async () => {
  ...
  it('should log Rebase with supply delta', async function () {
    const r = await policy.connect(bridge).rebase(2, 39992123);
    const t = await policy.rebaseTimestampSec();
    expect((async () => r)())
      .to.emit(policy, 'LogRebase')
      .withArgs(2, 0, t);
...
describe('XCAmpleforthController:rebase:orchestratorSuccess', async () => {
  beforeEach('setup XCAmpleforthController contract', async () => {
    ...
    await policy.setOrchestrator(mockOrchestrator.address);
  });
...
describe('XCAmpleforthController:rebase:orchestratorFailure', async () => {
  ...
  it('should revert rebase', async function () {
    await expect(policy.connect(bridge).rebase(2, 1000)).to.be.reverted;
  });
});
