...
describe('Orchestrator', function () {
  ...
  describe('when transaction list is empty', async function () {
    before('calling executePostRebaseCallbacks', async function () {
      await expect(await orchestrator.callStatic.executePostRebaseCallbacks()).to.be.true;
  ...
  describe.only('when a transaction reverts', async function () {
    ...
    it('should NOT revert', async function(){
      const call = await mockDownstream.populateTransaction.reverts()      console.log()
      await expect(await orchestrator.callStatic.executePostRebaseCallbacks()).to.be.false;
      await expect(orchestrator.connect(deployer).executePostRebaseCallbacks())
        .to.emit(orchestrator, 'TransactionFailed')
        .withArgs(mockDownstream.address, 1, call.data)
